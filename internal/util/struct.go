...
// Members of the struct such as empty strings or numbers set to zero
// are interpreted as a zero value!
// Parameter structPtr is a pointer to a struct.
func IsStructInitialized(structPtr interface{}) (err error) {
	...
	// we want to access values of the struct, not value of the pointer,
	// therefore we use Elem()
